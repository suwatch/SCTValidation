    ...
    internal static class SignedCertificateTimestampExtensions
    {
        internal static SctVerificationResult VerifySignature(this SignedCertificateTimestamp sct, Log logServer, IList<X509Certificate2> chain)
        {
            if (logServer == null || sct == null || chain?.Any() != true || logServer.LogId != sct.LogIdBase64)
                return SctVerificationResult.FailedVerification(sct.TimestampUtc, logServer?.LogId, logServer?.Description, "Invalid verification arguments");

            var nowUtc = DateTime.UtcNow;
            if (sct.TimestampUtc > nowUtc)
                return SctVerificationResult.FutureTimestamp(sct.TimestampUtc, logServer.LogId, logServer.Description);

            if (logServer.ValidUntilUtc.HasValue && sct.TimestampUtc > logServer.ValidUntilUtc)
                return SctVerificationResult.LogServerUntrusted(sct.TimestampUtc, logServer.LogId, logServer.Description);
            ...
            try
            {
                ...
                if (chain.Count < 2)
                    return SctVerificationResult.FailedVerification(sct.TimestampUtc, logServer.LogId, logServer.Description, "Chain with PreCertificate or Certificate must contain issuer");
                ...
                {
                    return SctVerificationResult.FailedVerification(sct.TimestampUtc, logServer.LogId, logServer.Description, "Chain with PreCertificate signed by PreCertificate Signing Cert must contain issuer");
                }
                else
                {
            ...
            {
                return SctVerificationResult.FailedWithException(sct.TimestampUtc, logServer.LogId, logServer.Description, ex);
            }
        }
